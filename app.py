...
import os
from openai import OpenAI  # GeminiからOpenAI互換ライブラリへ変更
from PIL import Image
import io
import time
import base64  # 画像をBase64変換するために追加
...
VISION_MODEL = "llama3.2-11b-vision"  # またはご利用のプロバイダが指定するモデル名
...
# ※GroqやTogether AIなど、利用するプロバイダに合わせて環境変数やBase URLを設定してください
if "LLAMA_API_KEY" in st.secrets:
 api_key = st.secrets["LLAMA_API_KEY"]
else:
 api_key = os.getenv("LLAMA_API_KEY", "")

# 必要に応じてプロバイダのBASE URLを設定（例: Groqの場合は https://groq.com）
base_url = os.getenv("LLAMA_API_BASE_URL", None)
...
def compress_image_to_base64(uploaded_file, max_size=(300, 300), quality=50):
 ...
 # OpenAI互換APIの仕様に合わせてBase64文字列に変換
...
【超重要：キャラクター設定】
以下の設定になりきり、各見出しの直後には必ず 1 行、キャラクターらしいセリフや挨拶を入れてから箇政書きを始めてください。
キャラクター：{char_choice}

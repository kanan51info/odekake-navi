...
# 出発時刻（5:00から30分刻みでプルダウンを作成）
time_options = []
start_time = datetime.strptime("05:00", "%H:%M")
for i in range(39):  # 5:00から24:00(0:00)、さらに24:30まで対応（5:00〜24:00は38枠）
...
if user_input or (uploaded_file is not None and not st.session_state.get("image_processed", False)):
    ...
    # APIへのシステムプロンプト (チケット料金算出、乗降場所詳細、出発時刻対応、トークン制限指示)
    system_prompt = f"""
...
・「今回は全体プランを教えて」などの全体要請がない限り、質問に関係のない「ルート案内」「お勧めSA」「気象リンク」などの定型項目は**絶対に表示しないでください**。
...
【厳守すべきルール・AIへの指示】
1. **【最重要：チケット料金と運賃の絶対提示】**
   乗り換えルートや交通手段を提示する際は、ユーザーがイメージしやすいように、**「片道の通常料金」「チケット代」「新幹線特急券＋乗車券の合計」などの運賃目安（例: 『新幹線〇〇線（東京〜京都）: 14,370円』等）を必ず算出して記載してください。** 料金を「要確認」で済ませることは厳禁です。
2. **【最重要：乗り降りの詳細スポット指定】**
   ルート案内を行う際は、以下の情報を徹底して記述してください。
   - **駅**を利用する場合: 駅名だけでなく、**「乗り口・改札名」および「番線・のりば番号」**まで必ず表示してください（例: 「東京駅 丸の内南口・21番線」「新宿駅 新南改札・4番線」）。
   - **空港**を利用する場合: 空港名だけでなく、**「ターミナル名」および「搭乗口（ゲート）番号・乗り口」**まで必ず表示してください（例: 「羽田空港 第2ターミナル・62番搭乗口」）。
3. **【最重要】固有名詞・数値を徹底的に具体化すること。**
   抽象的な表現（例：「地元の名物グルメ」「お得なきっぷ」「季節のイベント」など）は**禁止**します。
...
4. 不確実な最新の数値(リアルタイムな運行遅延時間など)は「要確認」として公式への動線を引いてください。それ以外の定番情報や通常料金は具体的に算出してください。
5. 以下の旅行情報および「指定出発時刻」を前提として、現実的な出発時間帯のルートを組み立ててください。
...
6. 交通機関に「自動車」または「バス」が含まれる場合、道中のおすすめサービスエリア(SA)や道の駅の「具体的な施設名」や「そこで食べるべきおすすめグルメ（店名・メニュー）」を必ず盛り込んでください。
7. 天災等のライフライン情報を検知した場合は、必ず冒頭に 【緊急速報】 とデカデカと赤文字 (マークダウンのHTML 表記等)で表示してください。
8. 出力結果は、要点を箇条書き等で分かりやすく整理し、冗長な挨拶や前置きは極限まで削ってください。
"""
